...
class ConfigUtil(context:Context) {
    ...
    companion object {
        ...
        private const val VAL_FILE_IS_FOLDER_FIRST = true
        private const val KEY_FILE_IS_SHOW_HIDDEN_FILE = "is_show_hidden_file"
        private const val VAL_FILE_IS_SHOW_HIDDEN_FILE = false
        private const val KEY_FILE_SORT = "sort"
        const val VAL_FILE_SORT_NAME = 0
        const val VAL_FILE_SORT_CREATE_TIME = 1
        const val VAL_FILE_SORT_MODIFY_TIME = 2
        const val VAL_FILE_SORT_SIZE = 3
    ...
    fun getFileIsFolderFirst():Boolean{
        return mFileConfig.getBoolean(KEY_FILE_IS_FOLDER_FIRST, VAL_FILE_IS_FOLDER_FIRST)
    }
    ...
    fun getFileIsShowHiddenFile():Boolean{
        return mFileConfig.getBoolean(KEY_FILE_IS_SHOW_HIDDEN_FILE, VAL_FILE_IS_SHOW_HIDDEN_FILE)
    }
    ...
    fun getFileSortOrder():Int{
        return mFileConfig.getInt(KEY_FILE_SORT, VAL_FILE_SORT_NAME)
    }
    fun setFileSortOrderAsName(){
        mFileConfig.edit().putInt(KEY_FILE_SORT, VAL_FILE_SORT_NAME).apply()
    }
    fun setFileSortOrderAsCreateTime(){
        mFileConfig.edit().putInt(KEY_FILE_SORT, VAL_FILE_SORT_CREATE_TIME).apply()
    }
    fun setFileSortOrderAsModifyTime(){
        mFileConfig.edit().putInt(KEY_FILE_SORT, VAL_FILE_SORT_MODIFY_TIME).apply()
    }
    fun setFileSortOrderAsSize(){
        mFileConfig.edit().putInt(KEY_FILE_SORT, VAL_FILE_SORT_SIZE).apply()
    }
    fun setFileSortOrderAsType(){
        mFileConfig.edit().putInt(KEY_FILE_SORT, VAL_FILE_SORT_TYPE).apply()
    }
    fun getFileIsOrderReverse():Boolean{
        return mFileConfig.getBoolean(KEY_FILE_IS_SORT_ORDER_REVERSE, VAL_FILE_IS_SORT_ORDER_REVERSE)
    }
